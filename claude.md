...
- **SPA Rendering Resilience**: For dynamic sites like D. E. Shaw, `services/scraper_service.py` enforces `networkidle` (15s timeout) followed by a 2-second hard jitter. It also incorporates a JS `document.body.innerText` fallback to ensure job descriptions are successfully extracted when standard DOM parsing via ATS selectors fails.
